...
class GAN(object):
    ...
    def generator(self, z, is_training=True, reuse=False):
        ...
        if self.dataset_name == 'mnist':
            ...
            with tf.variable_scope("generator", reuse=reuse):
                net = tf.nn.relu(bn(linear(z, 448 * 2 * 2, scope='g_fc1'), is_training=is_training, scope='g_bn1'))
                net = tf.reshape(net, [self.batch_size, 2, 2, 448])

                net = tf.nn.relu(
                    bn(deconv2d(net, [self.batch_size, 4, 4, 256], 4, 4, 2, 2, name='g_dc2'), is_training=is_training,
                       scope='g_bn2'))
                ...
                out = tf.nn.tanh(deconv2d(net, [self.batch_size, 32, 32, 3], 4, 4, 2, 2, name='g_dc5'))
